...
decision is made here. Every OS-visible name (data folder, mutex, Run value) comes from the constructor so tests never touch the real ones.
...
| `Common/ClipboardService.cs` | set and get an image, with the retries of a busy clipboard |
| `Common/ImageCodec.cs` | decode PNG/JPG/BMP (size checked first), encode PNG/JPG |
| `Common/FileStore.cs`, `FileLogger.cs`, `SystemClock.cs`, `TaskDelay.cs` | the small ports |
...
for example `IScreenSourcePort.Capture` -> `ScreenSource.Capture` -> `NativeMethods` -> a `PixelImage`.
