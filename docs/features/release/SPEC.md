...
- A release is started by the project owner, in one of two ways, and either leaves a draft on GitHub that the owner publishes by
  ...
  draft is published); the same run with "publish" ticked publishes the release at once when every check passed. An agent starts a
  release only when the owner says so in that session.
...
- Việc phát hành do chủ dự án bắt đầu, bằng một trong hai cách, cách nào cũng để lại một bản nháp trên GitHub cho chủ dự án bấm
  ...
  Cùng lần chạy đó mà chọn "đăng ngay" thì release được đăng công khai luôn khi mọi kiểm tra đạt. Agent chỉ bắt đầu một bản phát hành
  khi chủ dự án nói trong phiên đó.
