...
public sealed class CompositionRootTests
{
    ...
    public void EveryPortInteractorAndFlow_IsResolvableFromTheContainer()
    {
        ...
        StaHost.Instance.Invoke(() =>
        {
            ...
            Assert.That(provider.GetRequiredService<IScreenSource>(), Is.Not.Null);
            Assert.That(provider.GetRequiredService<IWindowCatalog>(), Is.Not.Null);
            Assert.That(provider.GetRequiredService<IMonitorCatalog>(), Is.Not.Null);
            Assert.That(provider.GetRequiredService<IClipboard>(), Is.Not.Null);
            Assert.That(provider.GetRequiredService<IAutostart>(), Is.Not.Null);
            ...
            Assert.That(provider.GetRequiredService<IHotkeys>(), Is.SameAs(provider.GetRequiredService<HotkeyService>()), "one hotkey service, so the container disposes the one that is used");
            Assert.That(provider.GetRequiredService<ISingleInstance>(), Is.SameAs(provider.GetRequiredService<SingleInstance>()));
